...
def menu(my_name,my_id):
	try:
		...
		print('[×] Cookies anda sudah basi ')
		time.sleep(5)
		login_lagi334()
	os.system('clear')
	banner()
	ip = requests.get("https://api.ipify.org").text
	gh = 'github.com/Alim-XD'
	cetak(nel('\tSelamat Datang [yellow]%s[white] Ngentod'%(my_name)))
	print(f'>> ID MU : '+str(my_id))
	print(f'>> IP MU  : {ip}')
	print(f'>> Github   : {gh}')
	print('')
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('>> Mau berapa id mas ? : '))
	except ValueError:
		print('>> Masukkan Angka, Jangan  Huruff ')
		exit()
	if jum<1 or jum>100:
...
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	print(f'{x}>> 1. Akun Yang Lebih Tua ')
	print('>> 2. Akun Yang Lebih Mudah ')
	print('')
	hu = input('>> Pilih : ')
	if hu in ['1','01']:
...
#		method.append('touch')
		...
		method.append('mobile')
	print('')
	_jembot_ = input('>> Tambahkan Aplikasi Yang Terkait ( Y/t ) ')
	if _jembot_ in ['']:
		print('>> Pilih Yang Bener ')
		back()
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[white]] Masukkan Katasandi Tambahan Minimal 6 Karakter\n[[cyan]•[white]] Contoh :[green] kakak,sayang,adik[white] '))
...
def passwrd():
	...
	print(f'>> Nyalakan Mode Pesawat Setiap {m}500{x} Idz\n')
